...
export function useDriverDetailViewModel(id: string | undefined) {
  ...
  useEffect(() => {
    ...
    let active = true
    getDriverDetail(id)
      .then((d) => { if (!active) return; setDetail(d); setError(null); setIsLoading(false) })
  ...
  const confirmReject = async (doc: ReviewDocument, motivo: string) => {
    if (doc.idDocumento == null) return
    try { await rejectDriverDocument(doc.idDocumento, motivo); notify.success('Documento rechazado.'); setRejectDoc(null); setTick((n) => n + 1) } catch (e) { notify.error(e) }
  }
